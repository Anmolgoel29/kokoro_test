...
def generate_first(text, voice='af_heart', speed=1, use_gpu=CUDA_AVAILABLE):
    ...
    for _, ps, _ in pipeline(text, voice, speed):
        ref_s = pack[len(ps)-1]
        print(f"Generating {len(ps)} tokens with speed {speed} using {'GPU' if use_gpu else 'CPU'}")
        try:
            if use_gpu:
...
def generate_all(text, voice='af_heart', speed=1, use_gpu=CUDA_AVAILABLE):
    ...
    for _, ps, _ in pipeline(text, voice, speed):
        ...
        print(f"\n\nGenerated {len(audio)/24000:.1f}s audio for {len(ps)} tokens \n\n")
...
def get_gatsby():
    with open('demo/gatsby5k.md', 'r') as r:
        return r.read().strip()

def get_frankenstein():
    with open('demo/frankenstein5k.md', 'r') as r:
        return r.read().strip()
