...
#     {"name": "DEBUG", "color": "<blue>", "level": 10},
#     {"name": "INFO", "color": "<green>", "level": 20},
#     {"name": "WARNING", "color": "<yellow>", "level": 30},
#     {"name": "ERROR", "color": "<red>", "level": 40},
#     {"name": "CRITICAL", "color": "<red><bold>", "level": 50},])
...
class KPipeline:
    ...
    def waterfall_last(
        ...
        for w in waterfall:
            ...
            if z < len(tokens) and tokens[z].phonemes in bumps:
                z += 1
            if next_count - len(KPipeline.tokens_to_ps(tokens[:z])) <= 240:
                return z
        return len(tokens)
    ...
    def generate_from_tokens(
        ...
        if isinstance(tokens, str):
            ...
            if len(tokens) > 240:
                raise ValueError(f'Phoneme string too long: {len(tokens)} > 240')
        ...
        for gs, ps, tks in self.en_tokenize(tokens):
            if not ps:
                ...
                logger.warning(f"Unexpected len(ps) == {len(ps)} > 240 and ps == '{ps}'")
                logger.warning("Truncating to 240 characters")
                ps = ps[:240]
    ...
    def __call__(
        ...
        for graphemes_index, graphemes in enumerate(text):
            ...
            if self.lang_code in 'ab':
                logger.warning(f"\n\n---->Processing English text: {graphemes[:50]}{'...' if len(graphemes) > 50 else ''}")
            ...
            # Non-English processing with chunking
                ...
                for chunk in chunks:
                    ...
                    if not ps:
                        ...
                        logger.warning(f'Truncating len(ps) == {len(ps)} > 240')
                        ps = ps[:240]
